...
test.describe('slash search', () => {
  test('should slash menu search and keyboard works', async ({ page }) => {
    ...
    await expect(slashItems.nth(1).locator('.text')).toHaveText(['2 Columns']);
    await expect(slashItems.nth(2).locator('.text')).toHaveText(['3 Columns']);
    await expect(slashItems.nth(3).locator('.text')).toHaveText(['Code Block']);
    await expect(slashItems.nth(4).locator('.text')).toHaveText(['Callout']);
    await expect(slashItems.nth(0)).toHaveAttribute('hover', 'true');
  ...
  test('slash menu supports fuzzy search', async ({ page }) => {
    ...
    await expect(slashItems.nth(4).locator('.text')).toHaveText(['2 Columns']);
    await expect(slashItems.nth(5).locator('.text')).toHaveText(['3 Columns']);
    await expect(slashItems.nth(6).locator('.text')).toHaveText(['Duplicate']);
    await expect(slashItems.nth(7).locator('.text')).toHaveText(['Code Block']);
    await expect(slashItems.nth(8).locator('.text')).toHaveText(['Linked Doc']);
    await expect(slashItems.nth(9).locator('.text')).toHaveText(['Attachment']);
    await type(page, 'b');
